...
function getTelemetryEventKey(event: DebugTelemetryEvent): string {
  return [
    ...
    event.previousFarmUpgradeLevel ?? "",
    event.nextFarmUpgradeLevel ?? "",
    event.farmSpeedMultiplier ?? "",
    event.farmFertilizerDoubleCropChancePercent ?? "",
    event.farmGeneratedQuantity ?? "",
    event.farmDoubleCropRolls ?? "",
